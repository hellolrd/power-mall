...
public class TokenTranslationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        String authorizationValue = request.getHeader(AuthConstant.AUTHORIZATION);
        // 判断是否有值
        if (StringUtils.hasText(authorizationValue)) {
            // 获取token
            String token = authorizationValue.replaceFirst(AuthConstant.BEARER, "");
            // 判断token是否有值
            if (StringUtils.hasText(token)) {
                // 解决token续签的问题
                // 从redis中获取token的存活时长
                Long expire = stringRedisTemplate.getExpire(AuthConstant.LOGIN_TOKEN_PREFIX + token);
                // 判断是否超过系统指定的阈值
                if (expire < AuthConstant.TOKEN_EXPIRE_THRESHOLD_TIME) {
                    // 给当前用户的token续签（本质就是增加token在redis中的存活时长）
                    stringRedisTemplate.expire(AuthConstant.LOGIN_TOKEN_PREFIX+token,AuthConstant.TOKEN_TIME, TimeUnit.SECONDS);
                }


                // 从redis中获取json格式字符串的认证用户信息
                String userJsonStr = stringRedisTemplate.opsForValue().get(AuthConstant.LOGIN_TOKEN_PREFIX + token);
